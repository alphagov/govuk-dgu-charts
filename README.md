...
10. On your local machine, update the `/etc/hosts` file as follows
...
- Check that the config map for `ckan-ckan-script` and `ckan-ckan-script-set` have the script settings that you expect to run with, if they have not been updated you can wait or click on `sync` on the `dgu-app-of-apps` project and on the `ckan` project to pick up the updates.
- Before the new job runs you will need to remove existing script job `kubectl delete job/ckan-ckan-script -n datagovuk`, otherwise the new job will fail with a sync error or will not run.
